##in this file, we will create the previous three year prediction for all clusters of each year. We will save the data for each year into separate files and will then use those features into our model.
...
def training(x_train):
    ...
    x_train = x_train.drop(['is_clinical','cited_by_clin','diffpctob','animal','diffpct','cluster','year','class','pct_is_newish','pctisnew_7'], axis = 1)
    ...
    model = LogisticRegression(random_state=0, C = 0.50).fit(x_train,y_train)
...
def match(proba, data, yr):
    ...
    ppprev = []
    for ind in data.index:
        x = proba[proba.cluster == data["cluster"][ind]]
        x1 = x[x.year == yr-1]
        if len(x1) == 0:
            prev.append(0)
        else:
...
def testing(x_test, model):
    ...
    temp = x_test
    x_test = x_test.drop(['prediction','year','cluster','biggest_anc','pct_in_biggest_anc','secbiggest_anc','is_clinical','cited_by_clin','pct_is_newish'], axis=1)
...
x_train = pd.read_csv("training_selected.csv")
#print(len(x_train))
data = pd.read_csv("features_prediction_1979.csv")
...
for i in yrs:
    ...
    train_temp = match(proba, data[data.year == i], i)
    train_final = train_final.append(train_temp, ignore_index= True)
    
train_final.to_csv("previous_years_1979.csv", index = False)
