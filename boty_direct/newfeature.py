...
def crossvalidation(x_train, y_train):
    ...
    for train, test in Kfold.split(x_train, y_train):
        model = LogisticRegression(random_state=0, C = 0.70).fit(x_train,y_train)
...
x_train = x_train.drop(['pct_in_secbiggest_anc','pct_of_secbiggest_anc','pct_of_secbiggest_anc_newish','rcr_low','cited_by_clin','prev'], axis = 1)
...
x_test = x_test.drop(['pct_in_secbiggest_anc','pct_of_secbiggest_anc','pct_of_secbiggest_anc_newish','rcr_low','cited_by_clin','prev'], axis=1)
...
temp.to_csv("prediction_LG_2020.csv", index = False)
...
while th <= 0.85:
    ...
    print("for threshold ",th," true covid data points ", len(true_covid)," recall ",len(true_byLR), " fp ",len(temp[temp.pred_log > th]) - len(true_byLR)," total pos ",len(temp[temp.pred_log > th]))
    th += 0.01
