...
def crossvalidation(x_train, y_train):
    ...
    for train, test in Kfold.split(x_train, y_train):
        model = LogisticRegression(random_state=0, C = 0.80).fit(x_train,y_train)
...
def calc_recall(y,data,th):
    ...
    '''true_high.to_csv("2008_true_high.csv", index = False)
    false_high.to_csv("2008_false_high.csv", index = False)
    true_low.to_csv("2008_true_low.csv", index = False)
    ...
    data2 = pd.read_csv("with250_training.csv")
...
def train(year,th):
    #year = 1984
    x_train = pd.read_csv("with250_training.csv")
    ...
    x_train = x_train.drop(['pct_in_secbiggest_anc','pct_of_secbiggest_anc','pct_of_secbiggest_anc_newish','rcr_low','cited_by_clin','class','prev'], axis = 1)
    ...
    data = pd.read_csv("with250_feature_"+str(year)+".csv")
    ####remove here
    data = cutoff(data)
    ...
    x_test = data
    #x_test = x_test.drop(['pctobnew_1','pctobnew_2','pctobnew_3','pctobnew_4','pctobnew_5','pctobnew_6'], axis=1)
    model = LogisticRegression(random_state=0, C = 0.80).fit(x_train,y_train) ##logistioc
    crossvalidation(x_train, y_train)
    '''
    ###svm
    model = svm.SVC(C=1.00)
    ...
    x_test = x_test.drop(['prediction','pct_in_secbiggest_anc','pct_of_secbiggest_anc','pct_of_secbiggest_anc_newish','rcr_low','cited_by_clin','prev'], axis=1) #$%
    ...
    #plt.show()
    temp_2 = temp
    temp_2 = temp_2.drop(['isgold'], axis = 1)
    temp_2.to_csv("prediction_LG_"+str(year)+".csv", index = False)
...
print("sum ", sum," percent ", sum/409)
print("Fp ", Fp," percent ", Fp/39)
print("FN ", FN)
print("EX ", ex)
#print("ss ", ss/34)
print("correct positive ", correct_pos)
print("total pos ", tot_pos/39)
